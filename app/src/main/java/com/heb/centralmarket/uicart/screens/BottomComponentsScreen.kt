...
@Composable
fun BottomComponentsScreen(
    navController: NavController,
    drawerState: DrawerState,
) {
    Scaffold(
        topBar = {
            AppBar(title = R.string.bottom_components, drawerState = drawerState, navigationIcon = {
        ...
        UICAppTheme {
            Surface {
                val componentsList =
                    listOf(
                        R.string.bottom_model_sheet,
                        R.string.bottom_button_container,
                    )
                CoreBackground {
                    LazyColumn(
                        modifier =
                        ...
                        itemsIndexed(componentsList) { index, component ->
                            ListItem(
                                modifier =
                                    Modifier
                                        .fillMaxWidth()
                                        .clickable {
                                            when(component){
                                                R.string.bottom_model_sheet -> {
                                                    navController.navigate(MainNavOption.BottomModelScreen.name)
                                                }
                                                R.string.bottom_button_container -> {
                                                    navController.navigate(MainNavOption.BottomButtonContainerScreen.name)
                                                }
                                            }
                                        },
                                headlineContent = {
